...
def parse_messages(restnode, messages):
    ...
    for message in messages:
        ...
        if message["message"].startswith("-----BEGIN ENCRYPTED KEY-----"):
        ...
        elif message["message"].startswith("-----BEGIN JSON-----"):
            messages[:] = [elem for elem in messages if not elem["message"].startswith("-----BEGIN JSON-----")]
            # me.remove_single_message_JSON(host)
            ...
            me.sendDigitalSignature(ListWithoutKeys, me)
            # me.remove_messages_block(host)
            parsed_messages += "" + "Wyslano JSON" + "\n"
            print(editedMessage)
        elif message["message"].startswith("-----BEGIN DIGITAL SIGNATURE-----"):
...
def updateChatbox():
    ...
    data = ""
    if read_from_block and len(me.chain.blocks) > 1:
        block = me.chain.blocks[last_block_index]
        '''
        if block.index != 0:
            data += "/\\/\\SPEC/\\/\\ Block number {0}\n".format(block.index)
        '''
        for messageData in block.data:
            blockOfData = messageData["message"]
            flatedList = "".join(blockOfData)
            decryptedBlock = decrypt_data_ecb(flatedList, me.useful_key)
            decryptedBlock += '\n'
            chat_history_from_blocks += decryptedBlock
        last_block_index += 1
    ...
    if len(json_messages) == 4:
        last_port = json_messages[-1]["port"]
        simpleMessage = json_messages[-1]
        if last_port == me.port:
            json_data = "-----BEGIN JSON-----\n"
            json_data += json.dumps(json_messages, sort_keys=True, separators=(',', ':'))
            me.sendBlockData(json_data)
        '''
        me.add_data(json_messages)
        me.remove_messages_block(host)
